...
import { buildOrganisationAdminHelp } from "@/lib/organisation-admin/build-help";
...
export async function GET(): Promise<NextResponse> {
  const guard = await requireOrganisationAdminRoute(ORG_ADMIN_SURFACE_PERMISSIONS.help);
  if (!guard.ok) return guard.response as NextResponse;
  try {
    const data = await buildOrganisationAdminHelp(guard.tenantId);
    if (!data) {

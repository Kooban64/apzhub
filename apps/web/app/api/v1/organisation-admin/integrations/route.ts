...
import { buildOrganisationAdminIntegrations } from "@/lib/organisation-admin/build-integrations";
...
export async function GET(): Promise<NextResponse> {
  ...
  try {
    const data = await buildOrganisationAdminIntegrations(guard.tenantId);
    if (!data) {
    ...
    return NextResponse.json(
      {
        error: {
          code: "ORG_ADMIN_INTEGRATIONS_FAILED",

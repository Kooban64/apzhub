...
import { buildOrganisationAdminSettings } from "@/lib/organisation-admin/build-org-settings";
...
export async function GET(): Promise<NextResponse> {
  ...
  try {
    const data = await buildOrganisationAdminSettings(guard.tenantId);
    if (!data) {
    ...
    return NextResponse.json(
      {
        error: {
          code: "ORG_ADMIN_SETTINGS_FAILED",

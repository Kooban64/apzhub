...
import { buildPlatformAdminTenantSubscription } from "@/lib/platform-admin/build-tenant-commercial";
...
export async function GET(
  ...
  try {
    const data = await buildPlatformAdminTenantSubscription(tenantId);
    if (!data) {
    ...
    return NextResponse.json(
      {
        error: {
          code: "TENANT_SUBSCRIPTION_FAILED",
          message:

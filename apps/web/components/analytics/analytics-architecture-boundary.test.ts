...
const SCAN_DIRS = [join(ROOT, "components/analytics"), join(ROOT, "lib/analytics")];

const FORBIDDEN_IMPORT_PATTERNS = [
  /@apzhub\/integration-metabase/,
...
describe("analytics UI architecture boundary", () => {
  it("does not import Metabase, gateways, or platform-services", () => {
    ...
    for (const file of files) {
      ...
      if (/\bmetabase\b/i.test(content) && !rel.endsWith("errors.ts")) {
        violations.push(`${rel}: contains metabase string`);
      }
    ...
    const apiSource = readFileSync(
      join(ROOT, "lib/analytics/analytics-api.ts"),
      "utf8",
    );
    expect(apiSource.includes('const API_BASE = "/api/v1"')).toBe(true);
    expect(apiSource.includes("@apzhub/integration-metabase")).toBe(false);

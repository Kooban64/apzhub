...
const SCAN_DIRS = [join(ROOT, "components/time"), join(ROOT, "lib/time")];
...
describe("time UI architecture boundary", () => {
  it("does not import engines, providers, gateways, or mapping stores", () => {
    ...
    for (const file of files) {
      ...
      if (
        /\bkimai\b/i.test(content) &&
        !rel.endsWith("errors.ts") &&
        !rel.endsWith("format.ts")
      ) {
        violations.push(`${rel}: contains kimai string`);
      }
    ...
    const apiSource = readFileSync(join(ROOT, "lib/time/time-api.ts"), "utf8");
    expect(apiSource.includes('const API_BASE = "/api/v1"')).toBe(true);
    expect(apiSource.includes("@apzhub/integration-kimai")).toBe(false);

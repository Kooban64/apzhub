...
const evidenceLifecycleStatusValues = [
  "pending",
  "captured",
  "submitted",
  "verified",
...
const normalizedResultStatusValues = [
  "pass",
  "fail",
  "skipped",
  "blocked",
...
const coverageMetricKindValues = [
  "requirement",
  "risk",
  "suite",
  "plan",
...
const defectProviderKindValues = [
  "internal",
  "projects",
  "support",
  "external_generic",
] as const;
const defectStatusValues = [
  "open",
  "in_progress",
  "resolved",
  "verified",
...
const traceabilityResourceTypeValues = [
  "requirement",
  "feature",
  "story",
  "task",
...
export const clonePlanBodySchema = z
  .object({
...
export const cloneCaseBodySchema = z
  .object({
    key: z.string().min(1).max(128).optional(),
...
export const testPlanCreateBodySchema = z
  .object({
    ...tenantInputSchema,
...
export const testSuiteCreateBodySchema = z
  .object({
    ...tenantInputSchema,
    key: z.string().min(1).max(128),
    name: z.string().min(1).max(255),
...
export const testSuiteUpdateBodySchema = testSuiteCreateBodySchema
  .omit({ tenantId: true, key: true })
  .partial()
  .strict()
  .refine((value) => Object.keys(value).length > 0, {
...
const testCaseParameterSchema = z
  .object({
    key: z.string().min(1).max(128),
...
export const testCaseCreateBodySchema = z
  .object({
    ...tenantInputSchema,
    key: z.string().min(1).max(128),
...
export const testCaseUpdateBodySchema = testCaseCreateBodySchema
  .omit({ tenantId: true, key: true })
  .partial()
  .strict()
  .refine((value) => Object.keys(value).length > 0, {
...
export const testCaseTransitionBodySchema = z
  .object({ status: z.enum(testStatusValues) })
  .strict();
...
export const requirementCreateBodySchema = z
  .object({
    ...tenantInputSchema,
    key: z.string().min(1).max(128),
    title: z.string().min(1).max(500),
...
export const requirementUpdateBodySchema = requirementCreateBodySchema
  .omit({ tenantId: true, key: true })
  .partial()
  .strict()
  .refine((value) => Object.keys(value).length > 0, {
...
const stepActualPayloadSchema = z
  .object({
...
export const executionCreateBodySchema = z
  .object({
    ...tenantInputSchema,
    ...
    stepActuals: z
      .array(stepActualPayloadSchema.extend({ stepId: platformIdSchema }))
      .max(500)
      .optional(),
...
export const executionAssignBodySchema = z
  .object({ assigneeId: platformIdSchema })
  .strict();
export const executionReasonBodySchema = z
  .object({ reason: z.string().min(1).max(10_000).optional() })
  .strict();
export const executionCompleteBodySchema = z
  .object({ overallResult: z.enum(testResultStatusValues).optional() })
  .strict();
export const executionCommentBodySchema = z
  .object({ comments: z.string().max(10_000).optional() })
  .strict();
export const executionRejectBodySchema = z
  .object({ comments: z.string().min(1).max(10_000) })
  .strict();
export const executionStepPatchBodySchema = stepActualPayloadSchema.refine(
  (value) =>
    value.status !== undefined || Object.keys(value).some((key) => key !== "status"),
  { message: "status or step actual fields are required." },
);

const evidenceRelationshipSchema = z
  .object({
...
export const evidenceRegisterBodySchema = z
  .object({
    ...tenantInputSchema,
...
export const evidenceVerifyBodySchema = z
  .object({ verificationState: z.string().min(1).max(128).optional() })
  .strict();
export const evidenceRejectBodySchema = z
  .object({ reason: z.string().min(1).max(10_000).optional() })
  .strict();

const canonicalEnvironmentSchema = z
  .object({
...
const canonicalCoverageSchema = z
  .object({
...
export const canonicalAutomationResultBodySchema = z
  .object({
...
export const automationImportBodySchema = z
  .object({
...
export const automationAggregateCoverageQuerySchema = z
  .object({ executionId: platformIdSchema.optional() })
  .strict();
...
export const coverageListQuerySchema = z
  .object({
    kind: z.enum(coverageMetricKindValues).optional(),
    planId: platformIdSchema.optional(),
    subjectId: platformIdSchema.optional(),
  })
  .strict();

function metricsFromJson(
  value: string,
  ctx: z.RefinementCtx,
  ...
  try {
    ...
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      message: "metrics must be valid JSON",
    });
    return {};
  }
}

export const qualityTrendsQuerySchema = z
  .object({
    ...
    baselineMetrics: z
      ...
      .min(2)
      .max(20_000)
      .transform(metricsFromJson)
      .optional(),
    currentMetrics: z.string().min(2).max(20_000).transform(metricsFromJson).optional(),
  })
  .strict();

export const qualityRegressionQuerySchema = z
  .object({ baselineSnapshotId: platformIdSchema, currentSnapshotId: platformIdSchema })
  .strict();

export const defectCreateBodySchema = z
  .object({
    ...tenantInputSchema,
...
export const defectUpdateBodySchema = defectCreateBodySchema
  .omit({ tenantId: true })
  .partial()
  .strict()
  .refine((value) => Object.keys(value).length > 0, {
...
export const defectLinkBodySchema = z
  .object({
    entityKind: z.union([z.enum(defectLinkTargetValues), z.string().min(1).max(128)]),
    entityId: platformIdSchema,
  })
  .strict();

export const certificationCreateBodySchema = z
  .object({
    ...tenantInputSchema,
    key: z.string().min(1).max(128),
    name: z.string().min(1).max(255),
...
export const certificationPrepareBodySchema = z
  .object({
    mode: z.enum(["plan", "certification"]),
    planId: platformIdSchema.optional(),
  })
  .strict();
export const certificationReasonBodySchema = z
  .object({ reason: z.string().min(1).max(10_000).optional() })
  .strict();
export const certificationRequiredReasonBodySchema = z
  .object({ reason: z.string().min(1).max(10_000) })
  .strict();
export const certificationConditionalApproveBodySchema = z
  .object({ conditions: z.string().min(1).max(10_000) })
  .strict();
export const releaseReadinessQuerySchema = z
  .object({ scope: z.enum(["plan", "certification"]).optional() })
  .strict();

export const traceabilityCreateBodySchema = z
  .object({
    ...tenantInputSchema,
    type: z.enum(traceabilityTypeValues),
    sourceKind: z.union([
      z.enum(traceabilityResourceTypeValues),
...
export const traceabilityResourceTypeParamSchema = z.enum(
  traceabilityResourceTypeValues,
);

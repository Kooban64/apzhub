...
describe("APZOBSERVE-003 observe client coverage", () => {
  ...
  it("covers HTTP diagnostics paths and error helpers", async () => {
    const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.includes("/diagnostics/readiness") || /\/observe\/readiness$/.test(url)) {
        return new Response(
          JSON.stringify({
            data: {
              ready: true,
      ...
      if (url.includes("/management-diagnostics") || url.includes("/capabilities")) {
        return new Response(
          JSON.stringify({
            data: {
              observeEnabled: true,
      ...
      if (url.includes("/diagnostics") && method === "POST") {
        return new Response(JSON.stringify({ data: { id: "pd_new", name: "n" } }), {
          status: 200,
        });
      }
      ...
      if (url.includes("/diagnostics/") && method === "GET") {
        return new Response(JSON.stringify({ data: { id: "pd_1", name: "d" } }), {
          status: 200,
        });
      }
    ...
    expect((await client.diagnostics.management()).managementPlaneReady).toBe(true);
    ...
    expect((await client.diagnostics.update("pd_1", { name: "n2" })).revision).toBe(2);
    ...
    expect((await client.healthChecks.list({ limit: 1 })).items[0]?.id).toBe("hc_1");

    await expect(client.healthChecks.get("x")).rejects.toBeInstanceOf(
      ObserveClientError,
    );

    expect(toObserveUserMessage(new ObserveClientError({ message: "m" }))).toBe("m");
    ...
    expect(observeQueryKeys.diagnostics.management()[1]).toBe("management-diagnostics");

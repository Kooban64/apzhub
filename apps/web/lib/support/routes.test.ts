...
describe("support routes", () => {
  ...
  it("parses detail ids and create routes", () => {
    ...
    expect(resolveSupportRoute(`${SUPPORT_BASE}/requests/new`)).toEqual({ kind: "create" });
  ...
  it("resolves all supported route kinds", () => {
    ...
    expect(
      resolveSupportRoute(`${SUPPORT_BASE}/organizations/sorg_eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee`),
    ).toEqual({
    ...
    expect(
      resolveSupportRoute(`${SUPPORT_BASE}/groups/sgrp_ffffffffffffffffffffffffffffffff`),
    ).toEqual({
      kind: "group-detail",
    ...
    expect(
      resolveSupportRoute(`${SUPPORT_BASE}/users/suser_11111111111111111111111111111111`),
    ).toEqual({
      kind: "user-detail",
      userId: "suser_11111111111111111111111111111111",
    });
    expect(resolveSupportRoute(`${SUPPORT_BASE}/search`)).toEqual({ kind: "search" });
    expect(resolveSupportRoute(`${SUPPORT_BASE}/analytics`)).toEqual({ kind: "analytics" });

...
| **Outbox**                      | `@apzhub/platform-outbox`                | **0.2.0**          | Reliable delivery (S08 CERT) | PCv2-02 · APZQEP-120-S08 — products MUST NOT invent private outbox engines        |
| **Processing**                  | `@apzhub/platform-processing`            | **0.1.1**          | Reliable execution (S09/S11) | Fan-out `resolveAll` for multi-bundle consumers                                   |
| **Quality Knowledge Index**     | `@apzhub/qep-knowledge-index`            | **0.1.0**          | Enterprise read model (S11)  | Event-driven projections; Search first consumer; never queries SoR                |

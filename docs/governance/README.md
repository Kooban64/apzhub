# APZHUB Enterprise Operating Model

> **Programme:** APZHUB-GOVERNANCE-001  
> **Title:** APZHUB Enterprise Operating Model  
> **Classification:** DOCUMENTATION ONLY  
> **Status:** **ACCEPTED / CLOSED** — Enterprise Operating Model  
> **Organisation:** **APZOR** operates **APZHUB**  
> **Production Baseline:** APZHUB Platform **1.1.0** (**ACCEPTED** · PRWL)  
> **Recommendation:** Accepted — next phase Commercial Execution  
> **Standard:** [Platform Delivery Standard](../engineering/platform-delivery/PLATFORM-DELIVERY-STANDARD.md)  
> **Date:** 2026-07-20  
> **Bootstrap:** AI-MANIFEST · repository evidence only  
> **Engineering:** **None authorised**

---

## Purpose

Define how **APZOR** governs people, decisions, products, risk, compliance, and continuous improvement around the APZHUB Production platform.

This is **enterprise governance** — not engineering delivery and not day-to-day platform operations runbooks.

| Layer                                                             | Authority                                                                                                    |
| ----------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ |
| Enterprise Operating Model                                        | **This framework** (`docs/governance/` EOM docs)                                                             |
| Platform Operations Framework                                     | [docs/operations/](../operations/README.md) **ACCEPTED**                                                     |
| Engineering Operating Model                                       | [docs/operations/ENGINEERING-OPERATING-MODEL.md](../operations/ENGINEERING-OPERATING-MODEL.md)               |
| Commercial Product Management                                     | [docs/product-management/](../product-management/README.md) **ACCEPTED**                                     |
| Platform Delivery Standard                                        | [docs/engineering/platform-delivery/](../engineering/platform-delivery/PLATFORM-DELIVERY-STANDARD.md)        |
| Engineering Governance Dashboard Spec (prior GOVERNANCE-001 wave) | [ENGINEERING-GOVERNANCE-DASHBOARD.md](./ENGINEERING-GOVERNANCE-DASHBOARD.md) **ACCEPTED** (spec only; no UI) |

---

## Enterprise Operating Model catalogue

| Document                       | Path                                                                                                                       |
| ------------------------------ | -------------------------------------------------------------------------------------------------------------------------- |
| Enterprise Operating Model     | [ENTERPRISE-OPERATING-MODEL.md](./ENTERPRISE-OPERATING-MODEL.md)                                                           |
| Organisational Model           | [ORGANISATIONAL-MODEL.md](./ORGANISATIONAL-MODEL.md)                                                                       |
| RACI                           | [RACI.md](./RACI.md)                                                                                                       |
| Roles and Responsibilities     | [ROLES-AND-RESPONSIBILITIES.md](./ROLES-AND-RESPONSIBILITIES.md)                                                           |
| Governance Committees          | [GOVERNANCE-COMMITTEES.md](./GOVERNANCE-COMMITTEES.md)                                                                     |
| Decision Making                | [DECISION-MAKING.md](./DECISION-MAKING.md)                                                                                 |
| Product Governance             | [PRODUCT-GOVERNANCE.md](./PRODUCT-GOVERNANCE.md)                                                                           |
| Portfolio Governance           | [PORTFOLIO-GOVERNANCE.md](./PORTFOLIO-GOVERNANCE.md)                                                                       |
| Change Governance              | [CHANGE-GOVERNANCE.md](./CHANGE-GOVERNANCE.md)                                                                             |
| Risk Governance                | [RISK-GOVERNANCE.md](./RISK-GOVERNANCE.md)                                                                                 |
| Compliance Governance          | [COMPLIANCE-GOVERNANCE.md](./COMPLIANCE-GOVERNANCE.md)                                                                     |
| Knowledge Governance           | [KNOWLEDGE-GOVERNANCE.md](./KNOWLEDGE-GOVERNANCE.md)                                                                       |
| AI Governance                  | [AI-GOVERNANCE.md](./AI-GOVERNANCE.md)                                                                                     |
| **AI Operational Framework**   | [APZHUB-AI-OPERATIONAL-FRAMEWORK.md](./APZHUB-AI-OPERATIONAL-FRAMEWORK.md) **IN FORCE** (FOUNDATION-002)                   |
| **Lifecycle Standard**         | [APZHUB-LIFECYCLE-STANDARD.md](./APZHUB-LIFECYCLE-STANDARD.md) **IN FORCE** (FOUNDATION-002)                               |
| **Engineering Standard**       | [APZHUB-ENGINEERING-STANDARD.md](./APZHUB-ENGINEERING-STANDARD.md) **IN FORCE** (FOUNDATION-002)                           |
| **Engineering Slice Standard** | [../engineering/ENGINEERING-SLICE-STANDARD.md](../engineering/ENGINEERING-SLICE-STANDARD.md) **IN FORCE** (APZHUB-ENG-001) |
| Continuous Improvement         | [CONTINUOUS-IMPROVEMENT.md](./CONTINUOUS-IMPROVEMENT.md)                                                                   |
| Meeting Cadence                | [MEETING-CADENCE.md](./MEETING-CADENCE.md)                                                                                 |
| Reporting Cadence              | [REPORTING-CADENCE.md](./REPORTING-CADENCE.md)                                                                             |
| Completion Report              | [COMPLETION-REPORT.md](./COMPLETION-REPORT.md)                                                                             |
| Acceptance Report              | [ACCEPTANCE-REPORT.md](./ACCEPTANCE-REPORT.md)                                                                             |

---

## Prior GOVERNANCE-001 wave (retained)

Engineering Governance Dashboard specification and companions remain binding for any future dashboard implementation — **no UI authorised**:

- [ENGINEERING-GOVERNANCE-DASHBOARD.md](./ENGINEERING-GOVERNANCE-DASHBOARD.md)
- [PORTFOLIO-STATUS-MODEL.md](./PORTFOLIO-STATUS-MODEL.md)
- [REPOSITORY-HEALTH-MODEL.md](./REPOSITORY-HEALTH-MODEL.md)
- [PROGRAMME-LIFECYCLE.md](./PROGRAMME-LIFECYCLE.md) · [CERTIFICATION-LIFECYCLE.md](./CERTIFICATION-LIFECYCLE.md)
- [GOVERNANCE-KPI-CATALOGUE.md](./GOVERNANCE-KPI-CATALOGUE.md)
- [README-ENGINEERING-GOVERNANCE.md](./README-ENGINEERING-GOVERNANCE.md)

---

## Explicit non-goals

Do **not** implement Release 1.2, redesign platforms, or implement monitoring/dashboards under this programme.

## Recommendation

# ENTERPRISE GOVERNANCE READY

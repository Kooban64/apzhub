# APZ Projects — Mission & Operational Pack

| Field           | Value                                         |
| --------------- | --------------------------------------------- |
| Product         | **APZ Projects**                              |
| Mission         | APZ-PROJECTS-000 **CLOSED** (Owner APPROVED)  |
| Native Adoption | APZ-PROJECTS-NATIVE-001 **COMPLETE / FROZEN** |
| Designation     | **REFERENCE IMPLEMENTATION #003**             |
| Timestamp       | 20260805T074500Z                              |

## Mission pack (PROJECTS-000)

| Deliverable  | Path                                               |
| ------------ | -------------------------------------------------- |
| Auth         | [OWNER-AUTHORISATION.md](./OWNER-AUTHORISATION.md) |
| Approval     | [OWNER-APPROVAL.md](./OWNER-APPROVAL.md)           |
| Mission      | [PRODUCT-MISSION.md](./PRODUCT-MISSION.md)         |
| Outcomes     | [BUSINESS-OUTCOMES.md](./BUSINESS-OUTCOMES.md)     |
| Metrics      | [SUCCESS-METRICS.md](./SUCCESS-METRICS.md)         |
| Stakeholders | [STAKEHOLDERS.md](./STAKEHOLDERS.md)               |
| Scope        | [PRODUCT-SCOPE.md](./PRODUCT-SCOPE.md)             |
| Out of scope | [OUT-OF-SCOPE.md](./OUT-OF-SCOPE.md)               |
| Value        | [VALUE-PROPOSITION.md](./VALUE-PROPOSITION.md)     |
| Vision       | [PRODUCT-VISION.md](./PRODUCT-VISION.md)           |
| Completion   | [COMPLETION.md](./COMPLETION.md)                   |

## Operational pack (N-04 — IN FORCE)

| Deliverable           | Path                                                                           |
| --------------------- | ------------------------------------------------------------------------------ |
| Quality lifecycle     | [APZPROJECTS-QUALITY-LIFECYCLE.md](./APZPROJECTS-QUALITY-LIFECYCLE.md)         |
| Release process       | [APZPROJECTS-RELEASE-PROCESS.md](./APZPROJECTS-RELEASE-PROCESS.md)             |
| Operational readiness | [APZPROJECTS-OPERATIONAL-READINESS.md](./APZPROJECTS-OPERATIONAL-READINESS.md) |
| Engineering checklist | [APZPROJECTS-ENGINEERING-CHECKLIST.md](./APZPROJECTS-ENGINEERING-CHECKLIST.md) |
| Quality checklist     | [APZPROJECTS-QUALITY-CHECKLIST.md](./APZPROJECTS-QUALITY-CHECKLIST.md)         |
| Release checklist     | [APZPROJECTS-RELEASE-CHECKLIST.md](./APZPROJECTS-RELEASE-CHECKLIST.md)         |
| Product status        | [PRODUCT-STATUS.md](./PRODUCT-STATUS.md)                                       |
| Product operations    | [PRODUCT-OPERATIONS.md](./PRODUCT-OPERATIONS.md)                               |
| Roles                 | [OPERATIONAL-ROLES.md](./OPERATIONAL-ROLES.md)                                 |
| Metrics               | [OPERATIONAL-METRICS.md](./OPERATIONAL-METRICS.md)                             |
| Learning              | [OPERATIONAL-LEARNING.md](./OPERATIONAL-LEARNING.md)                           |
| N-04 completion       | [COMPLETION-N04.md](./COMPLETION-N04.md)                                       |
| Reference designation | [REFERENCE-IMPLEMENTATION-003.md](./REFERENCE-IMPLEMENTATION-003.md)           |
| RI retrospective      | [RI-RETROSPECTIVE-003.md](./RI-RETROSPECTIVE-003.md)                           |

**REFERENCE IMPLEMENTATION #003** — fully adopted under APZQEP; third independent validation of the Playbook.

Native programme: [../apz-projects-native-001/](../apz-projects-native-001/)  
Playbook: [../framework/APZHUB-PRODUCT-ADOPTION-PLAYBOOK.md](../framework/APZHUB-PRODUCT-ADOPTION-PLAYBOOK.md)  
Capability model: [../apzhub-portfolio-001/ENTERPRISE-CAPABILITY-MODEL.md](../apzhub-portfolio-001/ENTERPRISE-CAPABILITY-MODEL.md)

Emerging patterns: [../framework/APZHUB-EMERGING-PORTFOLIO-PATTERNS.md](../framework/APZHUB-EMERGING-PORTFOLIO-PATTERNS.md)

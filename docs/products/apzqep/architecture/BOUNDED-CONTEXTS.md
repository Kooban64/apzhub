...
| **Identity** | User, role, session | Platform identity | *(platform)* |
...
| **Notification** | Attention item, subscription | Platform delivery | *(platform)* |
...
| **Invariants** | Executable procedures are approved library entries; *verification* not *test case* in API language |
...
| **Aggregate roots** | TraceLink *(link)*, CoverageSnapshot *(derived)* |
...
| **Aggregate roots** | Insight *(derived)* |
...
| **Aggregate roots** | InvestigationCase *(view)* |

...
| L-EM-01      | ~~list/search: permission + tenant scope only; no per-item ACL filter~~ **CLOSED** by **APZQEP-120-S01** (per-item ACL + tenant filter on enumeration) | **CLOSED**              | N/A (remediated)             | No                           |
...
| List without ACL filter               | **Remediated**  | Closed by APZQEP-120-S01 — secured facade applies getEvidence-equivalent ACL before list/search return |

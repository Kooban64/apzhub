...
| R-04 | Tenant-wide list/search for `qep.evidence.read` without per-item ACL filter (L-EM-01) | **Closed** | Remediated by **APZQEP-120-S01** (enumeration ACL on secured list/search) | Residual risk withdrawn; regression guarded by `security.enforcement.test.ts` / `enumeration-acl.test.ts` |

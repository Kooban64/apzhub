# APZ Support — Mission & Operational Pack

| Field           | Value                                        |
| --------------- | -------------------------------------------- |
| Product         | **APZ Support**                              |
| Mission         | APZ-SUPPORT-000 **CLOSED** (Owner APPROVED)  |
| Native Adoption | APZ-SUPPORT-NATIVE-001 **COMPLETE / FROZEN** |
| Designation     | **REFERENCE IMPLEMENTATION #002**            |
| Timestamp       | 20260805T045000Z                             |

## Mission pack (SUPPORT-000)

| Deliverable  | Path                                               |
| ------------ | -------------------------------------------------- |
| Auth         | [OWNER-AUTHORISATION.md](./OWNER-AUTHORISATION.md) |
| Approval     | [OWNER-APPROVAL.md](./OWNER-APPROVAL.md)           |
| Mission      | [PRODUCT-MISSION.md](./PRODUCT-MISSION.md)         |
| Outcomes     | [BUSINESS-OUTCOMES.md](./BUSINESS-OUTCOMES.md)     |
| Metrics      | [SUCCESS-METRICS.md](./SUCCESS-METRICS.md)         |
| Stakeholders | [STAKEHOLDERS.md](./STAKEHOLDERS.md)               |
| Scope        | [PRODUCT-SCOPE.md](./PRODUCT-SCOPE.md)             |
| Out of scope | [OUT-OF-SCOPE.md](./OUT-OF-SCOPE.md)               |
| Value        | [VALUE-PROPOSITION.md](./VALUE-PROPOSITION.md)     |
| Vision       | [PRODUCT-VISION.md](./PRODUCT-VISION.md)           |
| Completion   | [COMPLETION.md](./COMPLETION.md)                   |

## Operational pack (N-04 — IN FORCE)

| Deliverable           | Path                                                                         |
| --------------------- | ---------------------------------------------------------------------------- |
| Quality lifecycle     | [APZSUPPORT-QUALITY-LIFECYCLE.md](./APZSUPPORT-QUALITY-LIFECYCLE.md)         |
| Release process       | [APZSUPPORT-RELEASE-PROCESS.md](./APZSUPPORT-RELEASE-PROCESS.md)             |
| Operational readiness | [APZSUPPORT-OPERATIONAL-READINESS.md](./APZSUPPORT-OPERATIONAL-READINESS.md) |
| Engineering checklist | [APZSUPPORT-ENGINEERING-CHECKLIST.md](./APZSUPPORT-ENGINEERING-CHECKLIST.md) |
| Quality checklist     | [APZSUPPORT-QUALITY-CHECKLIST.md](./APZSUPPORT-QUALITY-CHECKLIST.md)         |
| Release checklist     | [APZSUPPORT-RELEASE-CHECKLIST.md](./APZSUPPORT-RELEASE-CHECKLIST.md)         |
| Product status        | [PRODUCT-STATUS.md](./PRODUCT-STATUS.md)                                     |
| Product operations    | [PRODUCT-OPERATIONS.md](./PRODUCT-OPERATIONS.md)                             |
| Roles                 | [OPERATIONAL-ROLES.md](./OPERATIONAL-ROLES.md)                               |
| Metrics               | [OPERATIONAL-METRICS.md](./OPERATIONAL-METRICS.md)                           |
| Learning              | [OPERATIONAL-LEARNING.md](./OPERATIONAL-LEARNING.md)                         |
| N-04 completion       | [COMPLETION-N04.md](./COMPLETION-N04.md)                                     |
| Reference designation | [REFERENCE-IMPLEMENTATION-002.md](./REFERENCE-IMPLEMENTATION-002.md)         |
| RI retrospective      | [RI-RETROSPECTIVE-002.md](./RI-RETROSPECTIVE-002.md)                         |

**REFERENCE IMPLEMENTATION #002** — fully adopted under APZQEP; second independent validation of the Playbook.

Native programme: [../apz-support-native-001/](../apz-support-native-001/)  
Playbook: [../framework/APZHUB-PRODUCT-ADOPTION-PLAYBOOK.md](../framework/APZHUB-PRODUCT-ADOPTION-PLAYBOOK.md)  
Emerging patterns: [../framework/APZHUB-EMERGING-PORTFOLIO-PATTERNS.md](../framework/APZHUB-EMERGING-PORTFOLIO-PATTERNS.md)

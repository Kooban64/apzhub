# APZ Time — Operational Adoption Pack

| Field     | Value               |
| --------- | ------------------- |
| Product   | **APZ Time**        |
| Slice     | TIME-NATIVE-001-A04 |
| Status    | **IN FORCE**        |
| Timestamp | 20260805T034500Z    |

Canonical operational governance for APZ Time under APZQEP.

| Deliverable           | Path                                                                   |
| --------------------- | ---------------------------------------------------------------------- |
| Quality lifecycle     | [APZTIME-QUALITY-LIFECYCLE.md](./APZTIME-QUALITY-LIFECYCLE.md)         |
| Release process       | [APZTIME-RELEASE-PROCESS.md](./APZTIME-RELEASE-PROCESS.md)             |
| Operational readiness | [APZTIME-OPERATIONAL-READINESS.md](./APZTIME-OPERATIONAL-READINESS.md) |
| Engineering checklist | [APZTIME-ENGINEERING-CHECKLIST.md](./APZTIME-ENGINEERING-CHECKLIST.md) |
| Quality checklist     | [APZTIME-QUALITY-CHECKLIST.md](./APZTIME-QUALITY-CHECKLIST.md)         |
| Release checklist     | [APZTIME-RELEASE-CHECKLIST.md](./APZTIME-RELEASE-CHECKLIST.md)         |
| Product status        | [PRODUCT-STATUS.md](./PRODUCT-STATUS.md)                               |
| Product operations    | [PRODUCT-OPERATIONS.md](./PRODUCT-OPERATIONS.md)                       |
| Roles                 | [OPERATIONAL-ROLES.md](./OPERATIONAL-ROLES.md)                         |
| Metrics               | [OPERATIONAL-METRICS.md](./OPERATIONAL-METRICS.md)                     |
| Learning              | [OPERATIONAL-LEARNING.md](./OPERATIONAL-LEARNING.md)                   |
| Completion            | [COMPLETION.md](./COMPLETION.md)                                       |
| Reference designation | [REFERENCE-IMPLEMENTATION-001.md](./REFERENCE-IMPLEMENTATION-001.md)   |

**REFERENCE IMPLEMENTATION #001** — fully adopted under APZQEP; template for future products.

Native programme: [../time/APZHUB-TIME-NATIVE-001/](../time/APZHUB-TIME-NATIVE-001/)  
Playbook: [../framework/APZHUB-PRODUCT-ADOPTION-PLAYBOOK.md](../framework/APZHUB-PRODUCT-ADOPTION-PLAYBOOK.md)

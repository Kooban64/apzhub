...
| G-21 | Permissions UI                | Default UI permissions hardcoded `["time.*"]` in router                                                           | Drive from APZHUB PermissionService / session              | UI not identity-bound                                                 | Requires Redesign         | No                                                           | **Critical** | **A02**       |
| G-22 | Identity / session            | Workbench uses `@apzhub/auth` session; Time UI does not own login                                                 | One APZHUB identity end-to-end; no Kimai login             | Login path OK at shell; Time permission wiring incomplete             | Wrapper                   | No                                                           | **Critical** | **A02**       |
...
| ----------------- | ------------------------------------------------------------------------ |
| **A02 Identity**  | G-21, G-22 (and settings that touch identity)                            |

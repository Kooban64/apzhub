# APZHUB Commercialisation & Go-To-Market Strategy

> **Programme:** APZHUB-STRATEGY-001  
> **Title:** APZHUB Commercialisation & Go-To-Market Strategy  
> **Classification:** DOCUMENTATION ONLY  
> **Status:** **ACCEPTED / CLOSED**  
> **Organisation:** **APZOR**  
> **Production Baseline:** APZHUB Platform **1.1.0** (**ACCEPTED** · PRWL)  
> **Recommendation:** **COMMERCIAL STRATEGY READY**  
> **Standard:** [Platform Delivery Standard](../../engineering/platform-delivery/PLATFORM-DELIVERY-STANDARD.md)  
> **Date:** 2026-07-20  
> **Bootstrap:** AI-MANIFEST · repository evidence only  
> **Engineering:** **None authorised**

---

## Purpose

Execution strategy for taking APZHUB to market: internal adoption, external customers, packaging, sales, success, launch, and revenue — **without** implementing billing, portals, marketing assets, or Release 1.2.

## Relationship to existing commercial frameworks

| Framework                                                          | Role                                                                                    |
| ------------------------------------------------------------------ | --------------------------------------------------------------------------------------- |
| [Product Management Framework](../../product-management/README.md) | Editions, licensing principles, personas, commercial catalogue (**ACCEPTED**)           |
| **This pack**                                                      | Commercial **execution** — GTM motions, sales/onboarding, launch, revenue roadmap, KPIs |
| [Enterprise Operating Model](../../governance/README.md)           | APZOR enterprise governance (**ACCEPTED**)                                              |
| [Operations Framework](../../operations/README.md)                 | Production support operations (**ACCEPTED**)                                            |
| Platform **1.1.0**                                                 | Sellable baseline under PRWL honesty                                                    |

**Rule:** No numeric prices in-repo (aligns with Product Management Pricing Strategy).

---

## Catalogue

| Document                   | Path                                                             |
| -------------------------- | ---------------------------------------------------------------- |
| Executive Strategy         | [EXECUTIVE-STRATEGY.md](./EXECUTIVE-STRATEGY.md)                 |
| Go-To-Market               | [GO-TO-MARKET.md](./GO-TO-MARKET.md)                             |
| Pricing Strategy           | [PRICING-STRATEGY.md](./PRICING-STRATEGY.md)                     |
| Packaging Strategy         | [PACKAGING-STRATEGY.md](./PACKAGING-STRATEGY.md)                 |
| Licensing Strategy         | [LICENSING-STRATEGY.md](./LICENSING-STRATEGY.md)                 |
| Customer Success           | [CUSTOMER-SUCCESS.md](./CUSTOMER-SUCCESS.md)                     |
| Implementation Methodology | [IMPLEMENTATION-METHODOLOGY.md](./IMPLEMENTATION-METHODOLOGY.md) |
| Sales Model                | [SALES-MODEL.md](./SALES-MODEL.md)                               |
| Partner Strategy           | [PARTNER-STRATEGY.md](./PARTNER-STRATEGY.md)                     |
| Marketing Strategy         | [MARKETING-STRATEGY.md](./MARKETING-STRATEGY.md)                 |
| Onboarding                 | [ONBOARDING.md](./ONBOARDING.md)                                 |
| Training                   | [TRAINING.md](./TRAINING.md)                                     |
| Launch Plan                | [LAUNCH-PLAN.md](./LAUNCH-PLAN.md)                               |
| Revenue Roadmap            | [REVENUE-ROADMAP.md](./REVENUE-ROADMAP.md)                       |
| KPIs                       | [KPIS.md](./KPIS.md)                                             |
| Success Measures           | [SUCCESS-MEASURES.md](./SUCCESS-MEASURES.md)                     |
| Risk Assessment            | [RISK-ASSESSMENT.md](./RISK-ASSESSMENT.md)                       |
| Completion Report          | [COMPLETION-REPORT.md](./COMPLETION-REPORT.md)                   |
| Acceptance Report          | [ACCEPTANCE-REPORT.md](./ACCEPTANCE-REPORT.md)                   |

## Explicit non-goals

Release 1.2 · platform redesign · marketing asset production · customer portals · billing/entitlement engines.

## Recommendation

# COMMERCIAL STRATEGY READY

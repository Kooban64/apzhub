...
import type { PlanePaginatedResponse, PlaneProjectRecord } from "../internal/plane-api-types";
...
import type { PageRequest, PageResult, ProjectListFilter, SortField } from "../models/query";
import {
  assertValid,
  mergeValidation,
  validatePageRequest,
...
import { validatePlanePaginatedResponse, validatePlaneProjectResponse } from "../validation/response-validation";
...
export class PlaneProjectService {
  ...
  async list(
    ...
    assertValid(
      mergeValidation(validatePageRequest(page), validateSortFields(sort, PROJECT_SORT_FIELDS)),
      "projects.list",
    );

    return this.deps.runner.run(context, "projects.list", async () => {
      const archived =
        filter.status === "archived" ? true : filter.status === "active" ? false : undefined;
  ...
  async get(context: IntegrationRequestContext, projectId: string): Promise<Project> {
    return this.deps.runner.run(context, "projects.get", async () => {
      const record = await this.deps.client.getProject(context, extractProjectPlaneId(projectId));
  ...
  async create(context: IntegrationRequestContext, input: CreateProjectInput): Promise<Project> {
    assertValid(
      mergeValidation(
    ...
    return this.deps.runner.run(context, "projects.update", async () => {
      const record = await this.deps.client.updateProject(
        ...
        mapProjectToPlaneBody({
          ...
          description: input.description,
          leadId: input.leadId,
          archived: input.status === "archived" ? true : input.status === "active" ? false : undefined,
        }),
      );
      assertValid(validatePlaneProjectResponse(record), "project.entity");
      return mapPlaneProject(record, {
  ...
  async archive(context: IntegrationRequestContext, projectId: string): Promise<Project> {
    return this.deps.runner.run(context, "projects.archive", async () => {
      const record = await this.deps.client.archiveProject(context, extractProjectPlaneId(projectId));
      assertValid(validatePlaneProjectResponse(record), "project.entity");
      return mapPlaneProject(record, {
        tenantId: this.deps.serviceContext.tenantId,
        workspaceId: this.deps.serviceContext.workspaceId,

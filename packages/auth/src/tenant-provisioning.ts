...
export async function provisionPlatformTenantForUser(userId: string): Promise<string> {
  if (process.env.DATABASE_URL) {
    try {
      const { ensureUserTenantMembership, seedDefaultPlatformTenantRow } = await import(
        "@apzhub/platform-identity/postgres"
      );
      await seedDefaultPlatformTenantRow();
      return await ensureUserTenantMembership({ userId, tenantId: DEFAULT_PLATFORM_TENANT_ID });
    } catch {
  ...
  service.assignUserToTenant({ userId, tenantId: DEFAULT_PLATFORM_TENANT_ID, isPrimary: true });

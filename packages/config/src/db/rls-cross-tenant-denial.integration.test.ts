...
beforeAll(async () => {
  postgresAvailable = Boolean(process.env.DATABASE_URL) && (await checkDatabaseHealth()).ok;
});

describe.runIf(postgresAvailable)("Law PostgreSQL RLS cross-tenant denial (TD-P10)", () => {
...
describe.runIf(!postgresAvailable)("Law PostgreSQL RLS cross-tenant denial availability", () => {

...
describe("observe-persistence postgres repositories", () => {
  it("maps all observability entity rows through mocked drizzle executor", async () => {
...

    {
      ...
      await repos.healthChecks.update(ctx, { ...entity, ...{"status":"degraded"} } as never);
    }

    {
      ...
      await repos.readinessChecks.update(ctx, { ...entity, ...{"status":"not_ready"} } as never);
    }

    {
      ...
      await repos.livenessChecks.update(ctx, { ...entity, ...{"status":"not_alive"} } as never);
    }

    {
      ...
      await repos.serviceHealth.update(ctx, { ...entity, ...{"overallStatus":"degraded"} } as never);
    }

    {
      ...
      await repos.serviceStatuses.update(ctx, { ...entity, ...{"message":"updated"} } as never);
    }

    {
      ...
      await repos.componentStatuses.update(ctx, { ...entity, ...{"status":"unhealthy"} } as never);
    }

    {
      ...
      await repos.metricDefinitions.update(ctx, { ...entity, ...{"status":"inactive"} } as never);
    }

    {
      ...
      await repos.metricSamples.update(ctx, { ...entity, ...{"valueLabel":"p95"} } as never);
    }

    {
      ...
      await repos.alertDefinitions.update(ctx, { ...entity, ...{"severity":"warning"} } as never);
    }

    {
      ...
      await repos.alertStates.update(ctx, { ...entity, ...{"state":"resolved"} } as never);
    }

    {
      ...
      await repos.dashboards.update(ctx, { ...entity, ...{"status":"inactive"} } as never);
    }

    {
      ...
      await repos.logSources.update(ctx, { ...entity, ...{"status":"archived"} } as never);
    }

    {
      ...
      await repos.traceDefinitions.update(ctx, { ...entity, ...{"status":"inactive"} } as never);
    }

    {
      ...
      await repos.traceSpans.update(ctx, { ...entity, ...{"spanName":"handler2"} } as never);
    }

    {
      ...
      const got = await repos.incidentReferences.get(ctx, asIncidentReferenceId("ir_pg"));
      expect(got?.title).toBe("Outage");
      expect(await repos.incidentReferences.list(ctx)).toHaveLength(1);
      await repos.incidentReferences.update(ctx, { ...entity, ...{"title":"Outage2"} } as never);
    }

    {
      ...
      const got = await repos.maintenanceWindows.get(ctx, asMaintenanceWindowId("mw_pg"));
      expect(got?.name).toBe("Patch");
      expect(await repos.maintenanceWindows.list(ctx)).toHaveLength(1);
      await repos.maintenanceWindows.update(ctx, { ...entity, ...{"status":"archived"} } as never);
    }

    {
      ...
      await repos.healthSummaries.update(ctx, { ...entity, ...{"healthyCount":4} } as never);
    }

    {
      ...
      await repos.diagnostics.update(ctx, { ...entity, ...{"detail":"warn"} } as never);
    }

    {
      ...
      await repos.metadata.update(ctx, { ...entity, ...{"status":"inactive"} } as never);
    }
  ...
  it("returns null when health check missing", async () => {
    ...
    const repos = createPostgresObserveRepositories(db);
    expect(
      await repos.healthChecks.get(ctx, asHealthCheckId("missing")),
    ).toBeNull();
  });
});

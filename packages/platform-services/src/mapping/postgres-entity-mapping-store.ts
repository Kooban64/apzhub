      ...
      async () => {
        try {
          ...
          if (filter.organisationId !== undefined) {
            conditions.push(
              eq(platformEntityMapping.organisationId, filter.organisationId),
            );
          }
          ...
          if (filter.integrationId) {
            conditions.push(
              eq(platformEntityMapping.integrationId, filter.integrationId),
            );
          }
          if (filter.status) {
          ...
          if (filter.parentPlatformId) {
            conditions.push(
              eq(platformEntityMapping.parentPlatformId, filter.parentPlatformId),
            );
          }
          ...
          return await this.db.transaction(async (tx) => {
            ...
            const nextParentProviderNativeId =
              input.parentProviderNativeId === null
                ? null
                : (input.parentProviderNativeId ??
                  existing.parentProviderNativeId ??
                  null);
  ...
  private async run<T>(
    ...
    try {
      ...
      this.logger.log({
        ...
        message: notFound
          ? `mapping.${operation}.not_found`
          : `mapping.${operation}.ok`,
      });
      this.metrics.record({
        operation,
        result: notFound ? "not_found" : "success",

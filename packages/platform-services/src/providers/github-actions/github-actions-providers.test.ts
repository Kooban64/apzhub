...
function ctx(): ServiceRequestContext {
  return {
    tenantId: "tenant-gha",
    organisationId: "org-gha",
    userId: "user-1",
    correlationId: "corr-gha-providers",
...
function createMockGitHubActionsCore(): GitHubActionsCoreServices {
...
describe("GitHub Actions providers", () => {
  it("registers and resolves repository/workflow/run providers", async () => {
    const registry = new ProviderRegistry();
    registerGitHubActionsProviders({
      registry,
      githubActionsCore: createMockGitHubActionsCore(),
    });

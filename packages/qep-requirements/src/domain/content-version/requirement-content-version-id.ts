...
export function createRequirementContentVersionId(value: string): RequirementContentVersionId {
  const normalized = value.trim();
  if (!/^rcv_[A-Za-z0-9_-]+$/.test(normalized)) {
    throw new QepInvariantViolation("Requirement content version id must start with rcv_");
  }

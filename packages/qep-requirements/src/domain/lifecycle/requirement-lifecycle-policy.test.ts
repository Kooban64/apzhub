...
describe("requirement lifecycle policy", () => {
  ...
  it("archives from rejected", () => {
    ...
    const archived = requirementLifecycleEngine.transition(
      rejected.newState,
      "archive",
      ctx,
    );
  ...
  it("blocks invalid transitions via guards", () => {
    expect(() =>
      requirementLifecycleEngine.transition("draft", "mark_verified", ctx),
    ).toThrow(LifecycleTransitionError);
    expect(() =>
      requirementLifecycleEngine.transition("rejected", "mark_implemented", ctx),
    ).toThrow(LifecycleTransitionError);
    expect(() =>
      requirementLifecycleEngine.transition("approved", "archive", ctx),
    ).toThrow(LifecycleTransitionError);
    expect(() =>
      requirementLifecycleEngine.transition("archived", "submit", ctx),
    ).toThrow(LifecycleTransitionError);
  });

  it("lists available transitions for the current state", () => {
    const fromProposed = requirementLifecycleEngine.availableTransitions(
      "proposed",
      ctx,
    );
    expect(fromProposed.map((item) => item.action).sort()).toEqual([
      "review",
      "start_review",
    ]);
  });

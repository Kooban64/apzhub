 ...
 * repositories, kept local so `@apzhub/qep-traceability` does not take a hard
...
export type RequirementsEndpointResolverDeps = {
 ...
 * Trace endpoint kinds against the Requirements bounded context. Any kind not
 * covered here (verification, evidence, defects, risk, certification, documents,
 * external references, etc.) resolves as not-found — compose with another
 * resolver (or extend) as those bounded contexts land.
 */
export function createRequirementsEndpointResolver(
  deps: RequirementsEndpointResolverDeps,
): TraceEndpointResolver {
  return {
    async resolve(tenantId, kind, artefactId): Promise<EndpointResolutionFact> {
      if (kind === "requirement") {

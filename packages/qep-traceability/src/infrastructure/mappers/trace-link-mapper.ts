...
export function traceLinkMatchesListFilters(
  ...
  if (query.sourceArtefactId && row.source.artefactId !== query.sourceArtefactId)
    return false;
  if (query.targetKind && row.target.kind !== query.targetKind) return false;
  if (query.targetArtefactId && row.target.artefactId !== query.targetArtefactId)
    return false;
  if (query.scopeReferenceId && row.scope.referenceId !== query.scopeReferenceId)
    return false;
  if (query.artefactId) {

...
describe("APZQEP-ENG-030C Traceability presentation routes", () => {
  it("recognises Traceability workspace routes", () => {
    ...
    expect(isQepTraceabilityRoute("/workspace/qep/traceability/trace-links")).toBe(
      true,
    );
  ...
  it("parses Trace Link ids and reserves new/supersede", () => {
    expect(
      parseQepTraceLinkRouteId("/workspace/qep/traceability/trace-links/trl_abc"),
    ).toBe("trl_abc");
    expect(
      parseQepTraceLinkRouteId(
        "/workspace/qep/traceability/trace-links/trl_abc/history",
      ),
    ).toBe("trl_abc");
    expect(
      isQepTraceHistoryRoute("/workspace/qep/traceability/trace-links/trl_abc/history"),
    ).toBe(true);
  ...
  it("exposes navigation contributions", () => {
    expect(QEP_TRACEABILITY_NAVIGATION.sidebar.href).toBe(
      "/workspace/qep/traceability",
    );

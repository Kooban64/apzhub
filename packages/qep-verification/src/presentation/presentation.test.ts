...
describe("APZQEP-ENG-040C Verification presentation routes", () => {
  it("recognises Verification workspace routes", () => {
    ...
    expect(isQepVerificationSearchRoute("/workspace/qep/verification/search")).toBe(true);
    expect(isQepVerificationHistoryRoute("/workspace/qep/verification/history")).toBe(true);
    expect(isQepVerificationDashboardRoute("/workspace/qep/verification/dashboard")).toBe(true);
  ...
  it("parses Verification ids and reserves named segments", () => {
    expect(parseQepVerificationRouteId("/workspace/qep/verification/ver_abc")).toBe("ver_abc");
  ...
  it("exposes navigation contributions", () => {
    expect(QEP_VERIFICATION_NAVIGATION.sidebar.href).toBe("/workspace/qep/verification");
    expect(QEP_VERIFICATION_NAVIGATION.additionalViews.length).toBeGreaterThanOrEqual(4);
  });
});

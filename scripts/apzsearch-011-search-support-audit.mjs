#!/usr/bin/env node
/**
 * APZSEARCH-011 — Support Search Publication Adapter audit.
...
const PKG = join(ROOT, "packages/search-support");
...
scan(files, [
  ...
  {
    rule: "no-zammad-package",
    pattern: /@apzhub\/integration-zammad/,
  },
  {
  ...
  },
  {
    ...
    pattern: /NextRequest|NextResponse|\/api\/v1\/search|workbench-framework|PlatformSearchView/,
  },
  {
...
if (pkgJson.name !== "@apzhub/search-support") {
  violations.push({
    file: "packages/search-support/package.json",
    line: 1,
    rule: "package-name",
...
for (const symbol of [
  ...
  "SupportSearchPublicationContext",
  ...
  if (!index.includes(symbol)) {
    violations.push({
      file: "packages/search-support/src/index.ts",
      line: 1,
      rule: "missing-export",
      detail: symbol,
    });
...
console.log(
  violations.length === 0
    ? "APZSEARCH-011 Support Search Publication Adapter audit PASSED"
    : "APZSEARCH-011 Support Search Publication Adapter audit FAILED",
);
console.log(`RESULT: ${violations.length === 0 ? "PASS" : "FAIL"}`);
console.log(`Violations: ${violations.length}`);
if (violations.length === 0) {
  console.log("  - @apzhub/search-support 0.1.0 → search-integration + platform-service-contracts only");
  console.log("  - No Meilisearch / platform-services / Zammad / HTTP / workers");
  console.log("  - Required SupportSearch* exports present");
} else {

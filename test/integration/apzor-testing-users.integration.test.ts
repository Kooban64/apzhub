...
/**
 * Same access profile as `scripts/seed-apzor-testing-users.ts` (applyHubDemoAccessForSubject);
 * uses a unique @apzor.com address so CI does not collide with a real seed roster.
 */
describe.skipIf(!hasDb)("apzor-style access profile (Postgres)", () => {
  it("materializes non-none roles for every catalog service and launch-ready posture when optimistic", async () => {
    await expect(
      getDb().transaction(async (tx) => {
        const email = `integration-apzor-${crypto.randomUUID()}@apzor.com`;
        const norm = email.toLowerCase();
        const hash = await argon2.hash("TestPassword-Apzor-12!", { type: argon2.argon2id });
        const [row] = await tx
          .insert(users)
          .values({
            email,
            emailNormalized: norm,
            displayName: "Apzor integration",
            status: "active",
        ...
        expect(directorySubjectIdForSession(snap)).toBe(row.id);
